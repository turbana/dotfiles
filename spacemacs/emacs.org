  ...
  (defun mu4e~headers-field-apply-basic-properties (msg field val width)
    (case field
      ...
      ((:maildir :path :message-id)
       (let* ((replacements '(("/gmail-professional/" . "pro/")
                              ("/gmail-personal/" . "per/")))
              (maildir (seq-reduce
                        (lambda (s replace)
                          (s-replace (car replace) (cdr replace) s))
                        replacements
                        val))
              (parts (split-string maildir "/"))
              )
         (concat (propertize (concat (car parts) "/")
                             'face 'mu4e-maildirs-extension-maildir-hl-face)
                 (propertize (mapconcat #'identity (cdr parts) "/")
...
In order
#+BEGIN_SRC emacs-lisp :tangle yes
  (defface ic/mu4e-test '((t :inherit default)) "doc" :group 'basic-faces)
