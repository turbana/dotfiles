...
#+BEGIN_SRC emacs-lisp :tangle yes
  (elfeed-org)
  (setq rmh-elfeed-org-files (list "~/org/home/elfeed.org"))

  (setq elfeed-search-title-max-width 400)
  (setq elfeed-search-title-min-width 16)
  (setq elfeed-search-trailing-width 50)
  ;; (clrhash elfeed-search--header-cache)

  (elfeed-goodies/setup)

  ;; show elfeed-goodies pane on bottom
  (setq elfeed-goodies/entry-pane-position 'bottom)
  (setq elfeed-goodies/wide-threshold 0.33)

  ;; stay on entry when performing a command
  ;; TODO change this to only when using a sidebar
  (setq elfeed-search-remain-on-entry t)

  ;; (setq elfeed-curl-extra-arguments '("-HUser-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:60.0) Gecko/20100101 Firefox/60.0"))
  ;; (setq elfeed-curl-extra-arguments nil)
#+END_SRC

*** Custom search display
I don't like the layout of either vanilla ~elfeed~ or ~elfeed-goodies~ so I'm going
to copy the display function form ~elfeed-goodies~ and customize it to my taste.

**** Custom Entry
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun ic/elfeed-search-print-entry (entry)
    ...
    (let* ((date (elfeed-search-format-date (elfeed-entry-date entry)))
           ...
           (title-width (- (window-width) elfeed-goodies/feed-source-column-width
                           elfeed-goodies/tag-column-width ic/elfeed-date-width 3))
  ...
  (setq ic/elfeed-date-width 10)
  (setq elfeed-search-print-entry-function #'ic/elfeed-search-print-entry)
  (setq elfeed-goodies/powerline-default-separator powerline-default-separator)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun search-header/draw-wide (separator-left separator-right search-filter stats db-time)
    (let* ((update (format-time-string "%Y-%m-%d %H:%M:%S %z" db-time))
           (lhs (list
                 (powerline-raw (-pad-string-to "Date" (- ic/elfeed-date-width 5)) 'powerline-active0 'l)
                 (funcall separator-left 'powerline-active0 'powerline-active1)
           ...
           (rhs (list
                 ...
                 (funcall separator-left 'powerline-active1 'powerline-active0)
                       ...
                       (cl-destructuring-bind (total-feeds queue-length in-progress) (-elfeed/queue-stats)
                         (setf content (concat content (format " (* %.0f%%%%)"
                                                               (* (/ (- total-feeds (+ queue-length
                                                                     ...
                                                                     total-feeds 1.0) 100))))))
                     (propertize content
                                 'face 'powerline-active0)))
                 ;; (funcall separator-right 'powerline-active1 'powerline-active0)
                 ;; (powerline-raw (concat " " update) 'powerline-active0 'r)
                 (funcall separator-right 'powerline-active0 'powerline-active1)
                 (powerline-raw (-pad-string-to "Tags" (- elfeed-goodies/tag-column-width 3)) 'powerline-active1 'l)
                 )))
      (concat (powerline-render lhs)
              (powerline-fill 'powerline-active2 (powerline-width rhs))
...
* Migrate
I plan on migrating from spacemacs to plain emacs. This section will facilitate that.

** Keybindings
Create an =org-mode= buffer containing all the current ky bind tables.

#+BEGIN_SRC emacs-lisp :tangle yes
  (defun ic//show-keybind-map (keymap keymap-name)
    "Print `map' to the current buffer as an `org-mode' table."
    ;; (unless (eq 'keymap (car map))
    ;;   (error "`map' must be a keymap"))
    (insert (format "* %s\n" keymap-name))
    (insert "| Key | Action |\n")
    (insert "|-----+--------|\n")
    (map-keymap
     (lambda (event value)
       (insert
        (format "| %s | %s |\n" (prin1-to-string event) value)))
     keymap)
    (org-table-align)
    )

  (defun ic//show-all-keybinds ()
    "Open the buffer `*ic-keybind-maps*' and display all key binding maps as
  `org-mode' tables."
    (interactive)
    (with-current-buffer (get-buffer-create "*ic-keybind-map*")
      (erase-buffer)
      (org-mode)
      (ic//show-keybind-map recentf-dialog-mode-map "recentf-dialog-mode-map")
      (switch-to-buffer-other-window "*ic-keybind-map*" t)
      )
    )
  (ic//show-all-keybinds)
  (local-set-key (kbd "<f7>") #'ic//show-all-keybinds)
#+END_SRC
