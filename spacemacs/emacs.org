...
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq ic/recipes-org-file "~/org/home/recipes/recipes.org")
  (setq ic/recipian-web-root "~/src/recipian-web/")
  (setq ic/github-pages-root "~/src/turbana.github.io")
  ...
  (defun ic/generate-recipes-site ()
    (interactive)
    (let ((debug-on-error t)
      ...
      ;; commit to git
      (when (eq (vc-git-state ic/recipian-recipes-json)
                'edited)
        (message "commiting recipes.json")
        (vc-git-checkin (list ic/recipian-recipes-json) "updated recipes.json"))
      ...
      (call-process (concat ic/node-bin-dir "npx")
                    ...
                    "--repo" (expand-file-name ic/github-pages-root)
                    "--branch" "master"
                    "--no-silent")
      ;; commit prod to github-pages
      (message "pushing to github pages...")
      (let ((default-directory ic/github-pages-root))
        (vc-git-push nil))
      (message "Recipe site generated")
      ))


  ;; XXX keybind during development
  (global-set-key (kbd "<f2>") 'ic/generate-recipes-site)
#+END_SRC
* Programming
** General
*** Indentation
