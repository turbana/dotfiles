...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun ic//is-recipe-p (elem)
    (let ((children
      ...
      (and (member "Ingredients" children)
           (member "Steps" children))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun ic//recipes-export-single (recipe)
    (message "!! %s" (org-element-property :raw-value recipe))
    (message "%S" (ic/org-element-tags recipe))
    )
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (let* ((variable '("Gentium" 13.0))
         (fixed    '("DejaVu Sans Mono" 11.0))
         (variable-size (truncate (* 10 (second variable))))
         (fixed-size (/ (second fixed) (second variable))))
    (custom-theme-set-faces
     'user
     `(variable-pitch ((t (:family ,(first variable) :height ,variable-size :weight normal))))
     `(fixed-pitch ((t (:family ,(first fixed) :height ,fixed-size))))))
#+END_SRC
** Variable pitch
...
#+END_SRC
** Line Numbers
Use relative line numbers everywhere.
#+BEGIN_SRC emacs-lisp :tangle yes
  (global-linum-mode)
