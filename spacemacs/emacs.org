...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (defun ic/watch-variable-print-stack (symbol &rest ops)
    ...
    (defun watch-variable-print-stack (symbol value operation buffer)
      (let ((stack-trace (with-output-to-string (backtrace))))
        (message "%s(%s): %s %S" symbol buffer operation value)
        (if (not stack-trace)
...
#+END_SRC
** Line Numbers
Use relative line numbers, but only in certain text-heavy modes.
#+BEGIN_SRC emacs-lisp :tangle yes
  (global-linum-mode -1)
...
Use =display-line-numbers= as the backend to =linum-relative= as it's written in C
rather than elisp that =linum= is. There is a measurable performance increase.
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq linum-relative-backend 'display-line-numbers-mode)
#+END_SRC

Show absolute line number for the current line, relative for others.
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq-default display-line-numbers-type 'visual
                display-line-numbers-current-absolute t)
#+END_SRC

Ignore folded lines when showing line number.
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq-default display-line-numbers-width nil
                display-line-numbers-widen t)
#+END_SRC

=linum-relative= this lovely bit of code in =(linum-relative-on)= found at
=linum-relative.el:215=
#+BEGIN_SRC emacs-lisp :tangle no
...
So it will clobber our version of =display-line-numbers-type= unless we also set
=linum-relative-user-type= to ='relative=. Ugh.
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq-default linum-relative-user-type 'relative)
#+END_SRC
