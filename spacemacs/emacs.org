...
#+END_SRC
** Spaceline
First we need to be able to load an icon with the correct font lock properties
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun ic/icon (name &optional group)
    (setq group (or group "alltheicon"))
    (let ((icon (funcall (intern (format "all-the-icons-%s" group)) name))
          (family (funcall (intern (format "all-the-icons-%s-family" group)))))
      (propertize icon
                  'face `(:family ,family :inherit)
                  'display '(raise 0.1))))
#+END_SRC

I want to see a VC icon along with branch name and status of current file.
#+BEGIN_SRC emacs-lisp :tangle yes
...
#+END_SRC
** Frame title
Set the frame title to something meaningful. =[layout name] :: [buffer name]=.
